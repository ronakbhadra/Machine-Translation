...
class EncoderLayer(nn.Module):
    def __init__(self,d_model,heads,dropout=0.1):
        ...
        self.LSTM = nn.LSTM(d_model,256,1,bidirectional=True,dropout = 0.1)
        
        self.linear = nn.Linear(2*256,256)
...
PATH = "modified_transformer_entire_model.pt"
...
modified_transformer_saved_model = pickle.dumps(model)
  
# Load the pickled model
test_model = pickle.loads(modified_transformer_saved_model)
...
#f = open("results.txt", "w",encoding="utf-8")
for i in range(4100,len(train_data)):
    print(i+1)
